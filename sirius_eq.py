...
# *** CORRECTED: Use wrap_at(360*u.deg) for 0-24h Hour Angle ***
...
def setup_scene():
    ...
    # *** NEW: Draw the Local Meridian in Magenta ***
    phi = np.linspace(0, np.pi, 100) # Half-circle from North to South
    y_m = SPHERE_RADIUS * np.cos(phi)
    z_m = SPHERE_RADIUS * np.sin(phi)
    ax.plot(np.zeros_like(y_m), y_m, z_m, color='magenta', lw=1.5, ls='-.')
    
    # Draw Equatorial Grid and Paths
    for dec in [-60, -30, 30, 60]:
        dec_rad = np.deg2rad(dec); r_dec = SPHERE_RADIUS * np.cos(dec_rad); z_dec = SPHERE_RADIUS * np.sin(dec_rad)
        pts = GLOBAL_TILT_MATRIX @ np.vstack([r_dec*np.cos(theta), r_dec*np.sin(theta), np.full_like(theta, z_dec)])
        ax.plot(pts[0,:], pts[1,:], pts[2,:], color='cyan', alpha=0.3, lw=0.75)
    ...
    for ra in np.arange(0, 180, 30):
        ...
        ra_rotated = rodrigues_rotation(base_circle, [0,0,1], ra_rad)
        final_pts = GLOBAL_TILT_MATRIX @ ra_rotated
        ax.plot(final_pts[0,:], final_pts[1,:], final_pts[2,:], color='cyan', alpha=0.3, lw=0.75)
    
    eq_pts = GLOBAL_TILT_MATRIX @ np.vstack([SPHERE_RADIUS*np.cos(theta), SPHERE_RADIUS*np.sin(theta), np.zeros_like(theta)])
    ax.plot(eq_pts[0,:], eq_pts[1,:], eq_pts[2,:], '--', color='red', lw=1.5)
    
    r_sirius_dec = SPHERE_RADIUS * np.cos(declination_rad)
    z_sirius_dec = SPHERE_RADIUS * np.sin(declination_rad)
    sirius_dec_circle_pts = GLOBAL_TILT_MATRIX @ np.vstack([r_sirius_dec*np.cos(theta), r_sirius_dec*np.sin(theta), np.full_like(theta, z_sirius_dec)])
    ax.plot(sirius_dec_circle_pts[0,:], sirius_dec_circle_pts[1,:], sirius_dec_circle_pts[2,:], color='green', lw=2)

    axis_line = np.array([-POLAR_AXIS_VEC * (SPHERE_RADIUS+2), POLAR_AXIS_VEC * (SPHERE_RADIUS+2)]).T
    ax.plot(axis_line[0], axis_line[1], axis_line[2], color='red', lw=1.5)
    ncp_pos = POLAR_AXIS_VEC * (SPHERE_RADIUS + 0.5)
    ax.text(ncp_pos[0], ncp_pos[1], ncp_pos[2], "NCP", color='red', ha='center')
...
# *** CORRECTED: path_plot now creates the yellow trace behind the star ***
path_plot, = ax.plot([], [], [], color='yellow', lw=2.5, zorder=6)
ha_text = ax.text2D(0.02, 0.05, '', transform=ax.transAxes, c='w', fontsize=12)
...
def update(frame):
    ...
    # CORRECTED: This now correctly traces the star's visible path from E to W
    ...
    N_POINTS_CIRCLE = 20; cyl_theta = np.linspace(0, 2*np.pi, N_POINTS_CIRCLE); z_cyl = np.linspace(0, TELESCOPE_LENGTH, 2)
    cyl_theta_grid, z_grid = np.meshgrid(cyl_theta, z_cyl)
    ...
    # Telescope mechanical simulation and final plotting
    dec_axis_template = np.array([[-1.5, 1.5], [0, 0], [0, 0]])
    # The telescope points at HA=0. To track, we rotate it opposite to the star's motion.
    # Star motion is increasing HA, so we rotate by -HA.
    ...
    # Calculate and draw the ray of light
    tip_template = np.array([[0],[0],[TELESCOPE_LENGTH]])
    tip_dec_locked = rodrigues_rotation(tip_template, [1, 0, 0], np.pi/2 - declination_rad)
    tip_ha_tracked = rodrigues_rotation(tip_dec_locked, [0, 0, 1], -current_ha_rad)
    tip_final = GLOBAL_TILT_MATRIX @ tip_ha_tracked
    ray_plot.set_data_3d([tip_final[0,0], star_x], [tip_final[1,0], star_y], [tip_final[2,0], star_z])

    return star_plot, ha_text, ray_plot, path_plot
